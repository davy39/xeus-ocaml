...
(** A mutable list to store rich outputs generated by user functions for the
    current cell execution. *)
let extra_outputs = ref []

(** Internal function to add an output to the global list. This is the core
    mechanism used by all public output functions. It prepends the output to the
    list.
    @param output The {!Protocol.output} value to add. *)
let add_output output = extra_outputs := output :: !extra_outputs
...
(** A generic helper to create a display data object with a single MIME type and
 ...
 *)
let output_display_data data = add_output (Protocol.DisplayData data)
...
let output_html s = create_and_add_display_data "text/html" (`String s)
...
let output_markdown s = create_and_add_display_data "text/markdown" (`String s)
 ...
 *)
let output_latex s = create_and_add_display_data "text/latex" (`String s)

(** Renders an SVG image from its XML string representation.
    @param s A string containing the full `<svg>...</svg>` XML markup. *)
let output_svg s = create_and_add_display_data "image/svg+xml" (`String s)
...
let output_png_base64 s = create_and_add_display_data "image/png" (`String s)

(** Displays a JPEG image from a Base64-encoded string.
    @param s The Base64-encoded string of the JPEG image data. *)
let output_jpeg_base64 s = create_and_add_display_data "image/jpeg" (`String s)

(** Displays a GIF image from a Base64-encoded string.
    @param s The Base64-encoded string of the GIF image data. *)
let output_gif_base64 s = create_and_add_display_data "image/gif" (`String s)
